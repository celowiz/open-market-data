...
`--start` and `--end` are inclusive. Resume is on by default: completed date windows whose `chunk_end` is at or before `state/backfill/bcb.json` `last_completed` are skipped. GitHub Actions restores that file best-effort; unlike B3, BCB does not treat `max(observation date)` as a chunk cursor (daily ingest would skip earlier 10-year windows). Raw JSON is stored per series and chunk at `raw/bcb/backfill/{code}/{chunk_start}_{chunk_end}.json`, with `:` in the series code replaced by `_` so local filesystem storage works on Windows (for example `BCB_CDI_DAILY`). Postgres `COMMIT`s after each 10-year chunk so a cancelled job keeps completed windows. The runner `./data` checkpoint does not survive between GitHub Actions jobs.
