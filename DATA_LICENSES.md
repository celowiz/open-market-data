...
| Yahoo Finance (via yfinance) | Yahoo / exchanges | no | Yahoo terms; yfinance is not a data license | UNKNOWN | unofficial client | UNKNOWN; default no | UNKNOWN; default no | n/a | yes, temporary (`public_api_enabled`) | no | `UNKNOWN` / API on, no Parquet | 2026-08-24 |
...
Phase 9 writes CVM, Tesouro, and BCB Parquet under `public/datasets/` with
those attribution snippets copied into each manifest. See
